...
const OnlineReshelveReport = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:8080/SYOS%20Backend/onlinereshelvereport')
      .then((response) => {
        if (!response.ok) {
  ...
  return (
    <div className="dashboard-container">
      ...
      <main className="main-content">
        <h2>Today's Online Reshelve Report</h2>
        {error ? (
          <p>{error}</p>
        ) : (
          <table>
...
export default OnlineReshelveReport;
