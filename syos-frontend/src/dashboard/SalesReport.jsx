...
const SalesReport = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:8080/SYOS%20Backend/salesreport')
      .then((response) => {
        if (!response.ok) {
  ...
  // Calculate totals
  const totalQuantity = reportData.reduce((sum, entry) => sum + entry.totalQuantity, 0);
  const totalRevenue = reportData.reduce((sum, entry) => sum + entry.totalRevenue, 0);
  ...
  return (
    <div className="dashboard-container">
      ...
      <main className="main-content">
        <h2>Today's Online Sales Report</h2>
        {error ? (
          <p>{error}</p>
        ) : (
          <table>
